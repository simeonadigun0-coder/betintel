...
function formatVolleyball(f, i) {
  return [
    `MATCH ${i + 1}: ${f.homeTeam} vs ${f.awayTeam} | ${f.country} — ${f.league} | KO: ${f.kickoff}`,
    `  HOME [${f.homeTeam}]: Record ${f.homeRecord}`,
    `  AWAY [${f.awayTeam}]: Record ${f.awayRecord}`,
  ].join("\n");
}
...
function buildTennisPrompt(fixtures, targetPicks, minConf) {
  ...
  return `You are a professional tennis betting analyst with deep knowledge of ATP and WTA tours, surfaces, and player styles. Today is ${today}.

You have REAL live data: ATP/WTA rankings, win/loss records, tournament round, and surface. USE THIS DATA as your primary source. Supplement with your knowledge of player styles, H2H, and surface preferences.
...
A) RANKING ANALYSIS
   — A ranking gap of 30+ positions is significant. Gap of 50+ is dominant.
   — However: surface specialist ranked 80 beating ranked 40 who hates that surface is common
   — Early rounds of Grand Slams and Masters: big gaps usually play out. Later rounds: anything goes
   — Lower ranked player on their best surface vs higher ranked player who struggles there = upset risk

B) SURFACE & STYLE CHECK
   — Clay: favours baseline grinders, high rallies, more games played, Overs more likely
   — Grass: favours big servers, fast points, short matches, Unders more likely
   — Hard: most balanced, check player-specific preferences
   — Know these players: identify serve-dominant vs baseline, aggressive vs defensive

C) TOURNAMENT CONTEXT
   — Early rounds (R128, R64, R32): heavy favourites almost always win, but games/sets vary
   — Later rounds (QF, SF, F): much more competitive, consider matches going to 3 sets
   — Grand Slams (best of 5 for men): total games markets are higher, more variance

D) THE 3-SIGNAL RULE
Must find 3 signals per pick:
  Match Winner: (1) significant ranking gap, (2) favourite's surface preference, (3) opponent's poor record on surface/round
  Over Games: (1) both players are baseliners/grinders, (2) clay surface, (3) close ranking suggests competitive match
If 3 signals not found → SKIP

AVAILABLE MARKETS — use EXACT text:
• "[Player Name] to Win" (use exact player name from fixture)
...
MARKET GUIDE:
→ Player to Win: ranking gap 40+, correct surface, good recent record — use when dominant
→ 2 Sets: huge ranking gap (50+) AND surface suits the favourite strongly
→ 3 Sets: close rankings, neutral surface, both players known for fighting back
→ Over Games: clay court + two baseliners + close rankings = long match
→ Under Games: grass/fast hard + at least one big server + ranking gap 30+ = short match
→ First Set Winner: when dominant player also known for fast starts, or opponent slow starter

RULES:
✓ Return 3 to ${targetPicks} picks — tennis is volatile, be selective
✓ Minimum ${minConf}% confidence
✓ Every pick needs 3 signals
✓ Star Pick = 90%+ — major ranking gap on preferred surface
✓ Be honest about surface uncertainty when surface is "Unknown"
✗ Never pick heavily on 3-set or 5-set matches without understanding the player styles
✗ Avoid upsets unless there is very specific contextual evidence (surface mismatch etc)

ANALYSIS: Name the player by their actual name. Quote rankings. Mention surface. Name 3 signals. State one risk.

Return ONLY a valid JSON array. Nothing else. No markdown.

[{"sport":"tennis","league":"ATP Tour","tournament":"Roland Garros","homeTeam":"Carlos Alcaraz","awayTeam":"Holger Rune","pick":"Carlos Alcaraz to Win","market":"Match Winner","probability":"79","confidence":"High","starPick":false,"analysis":"Three signals here. First, Alcaraz is ranked 2 in the world against Rune at 15 — a 13-place gap that on clay is very meaningful given how Alcaraz has dominated on this surface, winning Roland Garros twice. Second, clay strongly favours Alcaraz's game — his sliding, his forehand angle, and his ability to construct long points are all enhanced on this surface. Rune can compete on clay but has not shown the consistency to beat the very top players in best-of-five. Third, their H2H on clay shows Alcaraz winning the last two meetings comfortably. The risk is Rune's serve when it is working — he can disrupt rhythm and has the weapons to take a set if Alcaraz is slow to start.","kickoff":"13:00 UTC"}]`;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!apiKey)   return res.status(400).json({ error: "No Groq API key. Add GROQ_API_KEY to Netlify environment variables or enter it in Settings." });
