...
exports.handler = async (event) => {
  ...
  // How many seconds until midnight UTC — cache expires naturally at day rollover
  const now          = new Date();
  const midnight     = new Date(today);
  midnight.setDate(midnight.getDate() + 1);
  const secsToMidnight = Math.floor((midnight - now) / 1000);
  // Cache for the lesser of 6 hours or time until midnight
  const cacheTTL = Math.min(21600, secsToMidnight);

  const headers = {
    "Content-Type"               : "application/json",
    "Access-Control-Allow-Origin": "*",
    // ── Netlify CDN cache — serves this response for cacheTTL seconds
    // without invoking the function again. Saves ~90% of function minutes.
    "Cache-Control"              : `public, s-maxage=${cacheTTL}, stale-while-revalidate=300`,
    "Netlify-CDN-Cache-Control"  : `public, s-maxage=${cacheTTL}, stale-while-revalidate=300`,
  };
  ...
  try {
    ...
    for (let i = 0; i < FOOTBALL_LEAGUES.length; i += BATCH) {
      ...
      const res   = await Promise.all(batch.map(l => fetchLeague(l, dateStr)));
